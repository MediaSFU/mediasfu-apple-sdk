...
The hosted MediaSFU iOS runtime comes from the Kotlin Multiplatform `MediaSFUSDK` framework generated by `mediasfu-sdk-kotlin`. Swift consumers are expected to receive that as a synced prebuilt Apple artifact, not rebuild the Kotlin repo themselves.

This repo carries the companion native bridge layer and the Apple-side release sync helpers. See [KMP_BINARY_SYNC.md](KMP_BINARY_SYNC.md) for the artifact workflow.
...
This package ships the prebuilt `MediaSFUSDK` Apple binary for the current release, so Swift consumers do not need to rebuild `mediasfu-sdk-kotlin` locally.
